...
//        ];
	    ...
	    $driverkey = $account->getAccount('pushdriver');
        $app_key = $pushkey['account'];
        $master_secret = $pushkey['secret'];
	    $app_key1 = $driverkey['account'];
	    $master_secret1 = $driverkey['secret'];
        import('push.api.Push');
        $this->push = new \Push($app_key,$master_secret);
        $this->dpush = new \Push($app_key1,$master_secret1);
        $redis = new \Redis();
...
//        if(strtotime(date("Y-m-d")) ){
        ...
        foreach ($list as $key => $val){
            $online = $this->redis->get('onlined'.$val['did']);
//	        echo $this->redis->get('onlined'.$val['did']).' - '.time().'.';
            if(!empty($online)){
                if($this->redis->get('onlined'.$val['did']) <= time()){
                    $this->redis->set('onlinet'.$val['did'],'0');
	                $this->redis->del('onlined'.$val['did']);
                }
            }
        }
//        }
