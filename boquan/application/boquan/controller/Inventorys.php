...
use app\boquan\model\Check;
use app\boquan\model\Safety;
use app\boquan\model\Safenumber;
use app\boquan\model\Inventory;
...
class Inventorys extends Index
{
    ...
    //                $resarr = [];    // 存放返回数组
                    ...
                    if (!empty($_POST['time'])){
                        ...
                        $where['order_adtime'] = ['between' ,[$stime,$etime]];
                    }
                    if (!empty($_POST['sid'])){
                        $where['sid'] = $_POST['sid'];
                    }
                    ...
                    if (!empty($list)){
                        foreach ($list as $key => $val){
                            ...
                            $list[$key]['url'] = url('order_list_s',['id'=>$val['purchase_id']]);
                            $list[$key]['order_adtime'] = date('Y-m-d h:i',$val['order_adtime']);
                        }
                        $total = count($list);
